...
class fighter:
    ...
    def pickWeaponUser(self,RNG):
        weaponList = ["Iron Sword","Steel Sword","Silver Sword"]
        self.weapon = weaponList[RNG]
        #random.choice(weaponList)
        return self.weapon


    def pickWeaponSigurd(self,RNG):
    ...
    def pickVictory(self):
        ...
        return random.choice(victoryQuoteList)
    ...
    def getAttackState(self,RNG):
        #RNG = random.randint(0,99)
        if RNG < self.accuracy:
            self.attackState = " hits for " + str(self.damage)  + " damage!"
